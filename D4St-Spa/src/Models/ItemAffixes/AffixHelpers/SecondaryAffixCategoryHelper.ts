...
export class SecondaryTriggerAffixHelper {
    public GetByIndex(level:number, powerLevel:number, amount:number, chance:number, duration:number, type:number, trigger:ItemTriggerStats, damageType?:ResistanceTypesEnum):ItemAffixOutput {
        var delimiter = 4;
        var selected =
        ...
        var triggerStat = new ItemSecondaryTriggerStats(level, powerLevel, amount, chance, duration, selected, trigger, damageType);
