...
export class SkillsComponent implements OnInit, OnChanges {
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['levelUpModel']) {
      var updatedModel = changes['levelUpModel'].currentValue;
      console.log(this.levelUpModel);
    }
  }

  @Input() model: SkillWithImageVM;
  @Input() levelUpModel: SkillWithImageVM;
  ...
  constructor() {
    ...
    this.levelUpModel = new SkillWithImageVM(null, null, new SkillVM());
  ...
  async LevelUpPreview() {
    if (this.levelUpModel.skillData.skillData) {
      this.previewChanges = CalculationsHelper.calculateSkillDetails(this.model.skillLvlUpData, this.model.skillData);
    }
  }
  async LevelUpCancel() {
    this.previewChanges = new SkillDTO();
  ...
  async PreviewEmpower(power: number) {
    ...
    var model = this.model.skillData[selectedAffix] as SkillPowerDetailDTO;
    this.current = model.powerData;
    this.model.skillData[selectedAffix].powerData = model.powerUp;
    this.model.skillData[selectedAffix].powerData.Description = CalculationsHelper.GetSkillAffixDescription(model.powerUp, model.powerUp.AffixMetadata, power);
  }

  async CancelPreview(power: number) {
    var selectedAffix = power == 1 ? "angelicAffix" : power == 2 ? "demonicAffix" : "ancestralAffix";
    this.model.skillData[selectedAffix].powerData = this.current;
    var model = this.model.skillData[selectedAffix] as SkillPowerDetailDTO;
  }
}
